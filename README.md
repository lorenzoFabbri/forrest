...
the plot; `header` labels the row-label panel.
...
# Formatted text column shown alongside the plot
...
`weight` to scale point sizes by inverse-variance weights.
...
Set `dodge = TRUE` and supply a `group` column to stack multiple CIs
within each row — useful for comparing results across time periods,
sexes, models, or any other categorical dimension.
...
<img src="man/figures/README-dodge-1.png" alt="" width="100%" />
